...
class QAMLRoundFour:
    ...
    def select_questions(self, preamble_groups, standalone_questions, count_per_subject):
        ...
        return selected_questions[:count_per_subject * (len(preamble_groups) + 1)]
    ...
    def get_quiz_questions(self):
        ...
        for subject, subject_questions in categorized.items():
            ...
            selected = self.select_questions(preamble_groups, standalone_questions, 3)
            final_selection.extend(selected)

        random.shuffle(final_selection)

        return final_selection[:12]  # Ensure we only return 12 questions
